...
docker build -t streamyard/gstreamer:dev-dependencies -f Dockerfile-dev-dependencies .
# Download source code
docker build -t streamyard/gstreamer:dev-downloaded \
...
docker build --build-arg WEBKIT_USE_SCCACHE=$WEBKIT_USE_SCCACHE -t streamyard/gstreamer:$1-dev-with-source -f Dockerfile-dev-with-source .
# Build dev image with just binaries
docker build -t streamyard/gstreamer:$1-dev -f Dockerfile-dev .
# Build base production image with necessary dependencies
docker build -t streamyard/gstreamer:prod-base -f Dockerfile-prod-base .
# Build production image optimized binaries and no debug symbols (-O3 LTO)
docker build --build-arg WEBKIT_USE_SCCACHE=$WEBKIT_USE_SCCACHE -t streamyard/gstreamer:$1-prod -f Dockerfile-prod .
# Build production image optimized binaries and debug symbols
docker build --build-arg WEBKIT_USE_SCCACHE=$WEBKIT_USE_SCCACHE -t streamyard/gstreamer:$1-prod-dbg -f Dockerfile-prod-dbg .
